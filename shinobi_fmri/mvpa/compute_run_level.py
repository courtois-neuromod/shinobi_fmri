    ...
    for sub in subjects:
        ...
        for model in models:
            ...
            for cond in contrasts:
                output_fname = op.join("./", "reports", "figures", "subject-level", cond, "MVPA_fromruns", f"{sub}_{cond}_{model}_mvpa.png")
                os.makedirs(op.join("./", "reports", "figures", "subject-level", cond, "MVPA_fromruns"), exist_ok=True)
            ...
            confusion_matrices = []
            for train, test in decoder.cv.split(z_maps, contrast_label, groups=group_label):
                decoder.fit(np.array(z_maps)[train], np.array(contrast_label)[train], groups=np.array(group_label)[train])
            ...
            output_fname = op.join("./", "reports", "figures", "subject-level", "confusion_matrices_fromruns", f"{sub}_{model}_averaged_confusion_matrix.png")
            os.makedirs(op.join("./", "reports", "figures", "subject-level", "confusion_matrices_fromruns"), exist_ok=True)
            ...
            output_fname = op.join("./", "reports", "figures", "subject-level", "confusion_matrices_fromruns", f"{sub}_{model}_std_confusion_matrix.png")
            plt.savefig(output_fname)
            plt.close()
