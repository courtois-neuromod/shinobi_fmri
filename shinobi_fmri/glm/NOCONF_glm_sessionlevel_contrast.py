    ...
    if not os.path.exists(cmap_fname):
        ...
        for run in tqdm(sorted(runs)):
            ...
            if not run_events.empty:
                ...
                #bold_array = fmri_img.get_fdata()
                #assert bold_array.size == np.sum(np.reshape(np.reshape(bold_array, (-1, bold_shape[-1])).T.T, bold_shape) == bold_array) # Sanity check
                #boldchunk = np.reshape(clean(np.reshape(bold_array, (-1, bold_shape[-1])).T, detrend=True, standardize='zscore',
                #                           standardize_confounds=True, t_r=None, confounds=None, ensure_finite=True).T, bold_shape)
