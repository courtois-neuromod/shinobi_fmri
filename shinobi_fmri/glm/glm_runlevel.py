...
def create_output_folders(path_to_data, figures_path, contrast):
    ...
    for contrast in contrasts:
        if not os.path.isdir(op.join(path_to_data, 'processed', 'z_maps', 'run-level', contrast)):
            os.makedirs(op.join(path_to_data, 'processed', 'z_maps', 'run-level', contrast))
...
def compute_runlevel_glm(sub, ses, run, t_r=1.49, hrf_model='spm', savefigs=True):
    # Generate fnames
    fmri_fname = op.join(path_to_data, 'shinobi', 'derivatives', 'fmriprep-20.2lts', 'fmriprep', sub, ses, 'func', f'{sub}_{ses}_task-shinobi_run-{run}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz')
    anat_fname = op.join(path_to_data, 'anat', 'derivatives', 'fmriprep-20.2lts', 'fmriprep', sub, 'anat', f'{sub}_space-MNI152NLin2009cAsym_desc-brain_mask.nii.gz')
    events_fname = op.join(path_to_data, 'processed', 'annotations', f'{sub}_{ses}_run-0{run}.csv')
    ...
    if run_events.empty:
        ...
        confounds = Confounds(strategy=['high_pass', 'motion'],
                              ...
                              global_signal='full').load(fmri_fname)
        fmri_img = clean_img(fmri_fname, standardize=True,
                             detrend=True, high_pass=None,
                             t_r=t_r, ensure_finite=True,
                             confounds=None)
        ...
        design_matrix_raw = make_first_level_design_matrix(frame_times,
                                                    events=events_df,
                                                    drift_model=None,
                                                    hrf_model=hrf_model,
                                                    add_regs=confounds,
                                                    add_reg_names=None)
        regressors_clean = clean(design_matrix_raw.to_numpy(),
                                 detrend=True, standardize=True,
                                 high_pass=None, t_r=t_r,
                                 ensure_finite=True, confounds=None)
        design_matrix_clean = pd.DataFrame(regressors_clean, columns=design_matrix_raw.columns.to_list())
        ...
        if savefigs:
            design_matrix_raw_fname = op.join(figures_path, 'design_matrices', f'design_matrix_raw_{sub}_{ses}_run-0{run}.png')
            plotting.plot_design_matrix(design_matrix_raw, output_file=design_matrix_raw_fname)
            design_matrix_clean_fname = op.join(figures_path, 'design_matrices', f'design_matrix_clean_{sub}_{ses}_run-0{run}.png')
            plotting.plot_design_matrix(design_matrix_clean, output_file=design_matrix_clean_fname)
...
def plot_signals(z_map, fmri_img, fmri_fname):
    ...
    for i in range(0, 3):
        ...
        axs[0, i].set_title('Cluster peak {}\n'.format(coords[i])) # TODO : replace by test_statistic
        ...
        roi_img = plotting.plot_stat_map(
            ...
            axes=axs[1, i], display_mode='z', colorbar=False, bg_img=mean_img)
...
def process_run(sub, ses, run):
    ...
    glm_fname = op.join(path_to_data, 'processed', 'run-level', f'glm_{sub}_{ses}_{run}.pkl')
    if os.path.exists(glm_fname):
        ...
        try:
            print('Computing GLM')
            fmri_glm, fmri_img, fmri_fname = compute_runlevel_glm(sub, ses, run, t_r=t_r, hrf_model=hrf_model, savefigs=True)
        ...
        for contrast in contrasts:
            ...
            # Compute contrast
            z_map_fname = op.join(path_to_data, 'processed', 'z_maps', 'run-level', contrast, f'{sub}_{ses}_run-0{run}.nii.gz')
            if not(os.path.exists(z_map_fname)):
                ...
                # Save report
                report_fname = op.join(figures_path, 'run-level', contrast, f'{sub}_{ses}_run-0{run}_{contrast}_flm.html')
                ...
                view = plotting.view_img(clean_map, threshold=3, title=f'{contrast} (FDR<0.05), Noyaux > 10 voxels')
                view.save_as_html(op.join(figures_path, 'run-level', contrast, f'{sub}_{ses}_run-0{run}_{contrast}_flm_FDRcluster_fwhm5.html'))
                # Uncorrected image
                uncorr_map, threshold = threshold_stats_img(z_map, alpha=.001, height_control='fpr')
                view = plotting.view_img(uncorr_map, threshold=3, title=f'{contrast} (p<0.001), uncorr')
                view.save_as_html(op.join(figures_path, 'run-level', contrast, f'{sub}_{ses}_run-0{run}_{contrast}_flm_uncorr_fwhm5.html'))
                # Observed VS predicted values for top clusters
                fig_signals = plot_signals(z_map, fmri_img, fmri_fname)
                signals_plot_name = op.join(figures_path, 'run-level', contrast, f'signals_{sub}_{ses}_run-0{run}_{contrast}.png')
...
def main():
    ...
    for ses in sorted(sessions):
        ...
        runs = [filename[-12] for filename in os.listdir(op.join(path_to_data, 'shinobi', sub, ses, 'func')) if 'events.tsv' in filename]
